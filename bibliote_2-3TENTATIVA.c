...
void busca_porRA(aluno *p_aluno, char *ptr_ra_pesquisado);
...
void consultaAluno(aluno *p_aluno){
    ...
    case 3:
        ...
        scanf("%s", ptr_ra_pesquisado);
        printf("\n");
        busca_porRA(p_aluno, ptr_ra_pesquisado);
        break;
...
{
   FILE *fptr=NULL;
   int iterador_ra = 0, comparacao, qAluno;
   qAluno=verificaAluno();
...
{
    for (iterador_ra; iterador_ra < qAluno; iterador_ra++)
  {
               fssek(fptr,iterador_ra*sizeof(aluno),0);
               comparacao = strcmp((p_aluno+iterador_ra)->RA, ptr_ra_pesquisado);
               if(comparacao == 0){
                printf("Nome: %s", (p_aluno+iterador_ra)->nome);
                printf("\tRA: %s", (p_aluno+iterador_ra)->RA);
                int iterador_tabela=0;
                for(iterador_tabela; iterador_tabela < 4; iterador_tabela++){
                    printf("\t%c - ", (p_aluno+iterador_ra)->tabela[iterador_tabela].sigla);
                    printf("%i", (p_aluno+iterador_ra)->tabela[iterador_tabela].sigla);
            }
            fclose(fptr);
            printf("\n");
        }   
