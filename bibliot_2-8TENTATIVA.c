...
void busca_porRA(aluno *p_aluno,livro *p_livro);
int cadastrolivro_2(char num_ra[7], livro *p_livro);
void menu_livro(livro *p_livro);
      ...
      for(i=0;i<qAluno;i++)
      {
          ...
          { 
            ...
            printf("\n\n");
			 ...
			 {
				cadastrolivro_2(p_aluno->RA, p_livro);
			 }
          } 	
      }
       fclose(fptr);   	 
        ...
        if(strcmp(verifica_titulo, p_livro->titulo)==0)
		{
			for(j=0;j<2;j++)
			{
				if((p_livro->status+j)->sigla =='L' && flag==0)
				{
					...
					for(contVetor=0; contVetor<=7; contVetor++){
						(p_livro->status+j)->RA[contVetor] = num_ra[contVetor]; //PODE DAR ERRO -> pegar RA por paramentro da função
					}
					...
					if( (p_livro->status+j)->dia_ret + 7 > 31){
						...
						(p_livro->status+j)->mes_ret++;
						(p_livro->status+j)->dia_ret=conta;
					}else {
                ...
                if ((p_livro->status+j)->sigla =='E' && flag==0)
                {
                    	printf("Reserva realizada\n");
					(p_livro->status+j)->sigla = 'R';
                ...
                if ((p_livro->status+j)->sigla =='R' && flag==0)
                {
