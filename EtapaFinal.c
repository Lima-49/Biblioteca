...
int deleta_livro(livro *p_livro);
...
{
  char  verifica_titulo[20], num_ra[7];
  	int  i,j,qlivro, qAluno, contVetor, conta, flag=0, cont, contAluno, somaE, multa, diaAtual, mesAtual,contadev;
  	qlivro= verificaLivro();
	qAluno = verificaAluno();
  	FILE *fptr=NULL;
	FILE *fptrAluno=NULL;
    ...
    if(p_aluno){
		...
		fflush(stdin);
       	...
       	printf("\nDigite a data do livro a ser devolvido.  Exemplo: 20 2\n");
        scanf("%i %i", &diaAtual, &mesAtual);
		...
		if((fptr=fopen("livro.bin","rb"))==NULL){
			...
			for(i=0;i<qlivro;i++){
				...
				if(strcmp(verifica_titulo, p_livro->titulo)==0){
					if(strcmp((p_livro->status+0)->RA, p_aluno->RA ) == 0){
						contadev= mesAtual-(p_livro->status)->mes_dev;
						
						if(contadev>0){
							if((p_livro->status)->mes_dev==2){
								conta=diaAtual-(p_livro->status)->dia_dev+28;                                    
								multa=conta*3;

							}else if((p_livro->status)->mes_dev==1 || (p_livro->status)->mes_dev==3 || (p_livro->status)->mes_dev==5 || (p_livro->status)->mes_dev==7 || (p_livro->status)->mes_dev==9 || (p_livro->status)->mes_dev==11){
								conta=diaAtual-(p_livro->status)->dia_dev+31;                                    
								multa=conta*3;

							}else if((p_livro->status)->mes_dev==4 || (p_livro->status)->mes_dev==6 || (p_livro->status)->mes_dev==8 || (p_livro->status)->mes_dev==10 || (p_livro->status)->mes_dev==12){
								conta=diaAtual-(p_livro->status)->dia_dev+30;
								multa=conta*3;
							}


						}else if(contadev==0){
							conta=diaAtual-(p_livro->status)->dia_dev;
							multa=conta*3;
							
						}else {
							deleta_livro(p_livro);
                            
						}
						
                        //MUDAR O STATUS[1] PARA STATUS[0] DE RESERVADO PARA EMPRESTADO
						
					} else {
						//CASO DIGITOU NOME DO LIVRO ERRADO
						
						printf("Livro não foi encontrado");
					
					}
					printf("conta: %i",conta);
					printf("O valor da sua multa: R$ %i,00",multa);
...
{
	char titulo[80];
	int i, qlivro, compara,k=0,j, regis;
	qlivro= verificaLivro();
	printf("\nQual o Titulo a ser Buscado: ");
	scanf("%s", titulo);
	fflush(stdin);
	FILE *fptr=NULL;
	if((fptr=fopen("livro.bin","rb"))==NULL){
		...
		for(i=0;i<qlivro;i++){
			fseek(fptr,i*sizeof(livro),0);
			fread(p_livro,sizeof(livro),1,fptr);
			compara = strcmp(p_livro->titulo, titulo);
			if(compara==0){ 
				printf("\nRegistro: %i	Titulo: %s	Autor:	%s	", p_livro->reg, p_livro->titulo, p_livro->autor);    
				for(j=0; j<2; j++){
					printf(" Sigla: %c RA: %s Retirada: %i/%i Devolucao: %i/%i", (p_livro->status+j)->sigla, (p_livro->status+j)->RA, (p_livro->status+j)->dia_ret, (p_livro->status+j)->mes_ret, (p_livro->status+j)->dia_dev, (p_livro->status+j)->mes_dev );
				...
				scanf("%d", &regis);
				fflush(stdin);
				if((p_livro->status)->sigla =='E'){
					p_livro->titulo[80] = '@';					
					printf("Multa de R$150,00\n");
				}
				else{
	 ...
	 fclose(fptr); 
	 //gravaLivro(p_livro,"rb+", p_livro->titulo);
    }
}