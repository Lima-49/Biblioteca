  ...
  char  verifica_titulo[20], num_ra[7];
				...
				for(i=0;i<qlivro;i++){
					...
					if(strcmp(verifica_titulo, p_livro->titulo)==0){
                        ...
                        printf("Insira o dia e mes atual (31 2): \n");
							...
							}if ((p_livro->status+j)->mes_ret == 4 || (p_livro->status+j)->mes_ret == 6|| (p_livro->status+j)->mes_ret == 8 || (p_livro->status+j)->mes_ret == 10 || (p_livro->status+j)->mes_ret == 12 ){
								if ((p_livro->status+j)->dia_ret + 7 > 31){
									conta=(p_livro->status+j)->dia_ret + 7 -31;
									(p_livro->status+j)->dia_dev = conta;
									(p_livro->status+j)->mes_dev = (p_livro->status+j)->mes_ret +1;
   ...
   /*if((fptr=fopen("livro.bin","rb"))==NULL){
				...
				for(i=0;i<qlivro;i++){
					...
					fseek(fptr,i*sizeof(livro),0);
					fread(p_livro,sizeof(livro),1,fptr);
                    if((p_livro->status) == 'R' && strcmp(verifica_titulo, p_livro->titulo)==0)
                    {
	...
	if((fptr=fopen("livro.bin","rb"))==NULL){
		...
		for(i=0;i<qlivro;i++){
			...
			if(compara==0){ 
				...
				scanf("%d", &regis);
				if((p_livro->status+k)->sigla =='E'){
						printf("Multa de R$150,00");
				}
				else{
				puts("Livro nao emprestado");
				}		
			}
