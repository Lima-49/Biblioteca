...
void emprestaLivro2(livro *p_livro);
...
int emprestaLivro( livro *p_livro){
	...
	char  verifica_titulo[20], num_ra[7];
  	int  i,j,qlivro, contVetor, conta, flag=0;
            ...
            if((fptr=fopen("livro.bin","rb"))==NULL){
				...
				for(i=0;i<qlivro;i++){
					...
					if(strcmp(verifica_titulo, p_livro->titulo)==0){
						...
						flag=0;
						if((fptr=fopen("livro.bin","w+b"))==NULL){
      						printf("\nErro ao abrir o arquivo");
  	        			}else {
							j=0;
							
							if((p_livro->status+j)->sigla =='L' && flag==0){
								...
								if((p_livro->status+j)->mes_ret==2){
									if ((p_livro->status+j)->dia_ret + 7 > 28){
										conta=((p_livro->status+j)->dia_ret + 7 > 28)-28;
									...
									if ((p_livro->status+j)->dia_ret + 7 > 30){
											conta=((p_livro->status+j)->dia_ret + 7 > 30)-30;
											(p_livro->status+j)->dia_dev = conta;
											(p_livro->status+j)->mes_dev = (p_livro->status+j)->mes_ret +1;
									...
									if ((p_livro->status+j)->dia_ret + 7 > 31){
										conta=((p_livro->status+j)->dia_ret + 7 > 31)-31;
										(p_livro->status+j)->dia_dev = conta;
										(p_livro->status+j)->mes_dev = (p_livro->status+j)->mes_ret +1;
							...
							} else if ((p_livro->status+j)->sigla =='E' && flag==0){
								if((p_livro->status+1)->sigla =='R'){
									...
									{
										conta=((p_livro->status+1)->dia_ret + 7 > 28)-28;
								...
								}else if ((p_livro->status+1)->mes_ret == 1 || (p_livro->status+1)->mes_ret == 3 || (p_livro->status+1)->mes_ret == 5 || (p_livro->status+1)->mes_ret == 7 || (p_livro->status+1)->mes_ret == 9 || (p_livro->status+1)->mes_ret == 11)
								{
									if ((p_livro->status+1)->dia_ret + 7 > 30)
										{
											conta=((p_livro->status+1)->dia_ret + 7 > 30)-30;
											(p_livro->status+1)->dia_dev = conta;
											(p_livro->status+1)->mes_dev = (p_livro->status+1)->mes_ret +1;
									...
									if ((p_livro->status+1)->dia_ret + 7 > 31)
										{
											conta=((p_livro->status+1)->dia_ret + 7 > 31)-31;
											(p_livro->status+1)->dia_dev = conta;
											(p_livro->status+1)->mes_dev = (p_livro->status+1)->mes_ret +1;
								...
								flag++;
							} else if((p_livro->status+1)->sigla =='R' && flag==0){
								printf("Livro já reservado\n");
								
								
								flag++;
							}

							fwrite(p_livro, 1 , sizeof(livro), fptr);
...
void emprestaLivro2( livro *p_livro){
	...
	char numRA[7], titulo[80];
	int q_livro = verificaLivro(), i;
	aluno *p_aluno;
	//livro *p_livro;
	FILE *fptr=NULL;
	iniciaaluno(&p_aluno, 1);
	...
	printf("Digite o RA do aluno q vai emprestar");
	scanf("%s", numRA);
	p_aluno = verificaRA(numRA);

	if(p_aluno->emprestado < 3 && p_aluno->reservado < 1){
		//CASO ALUNO POSSA EMPRESTAR
		printf("\nQual livro deseja emprestar: ");
		scanf("%s", titulo);

		fptr = fopen("livro.bin", "rb");
		...
		if(fptr){
			...
			for(i; i<q_livro; i++){
				...
				if(strcmp(titulo, p_livro->titulo)==0){
					//CASO ENCOTROU O TITULO
					if(p_livro->status->sigla == 'L'){
						p_livro->status->sigla = 'E';
						strcpy(p_livro->status->RA, numRA);
						//COLOCAR DATA
					} else if(p_livro->status->sigla == 'E'){
						if((p_livro->status+1)->sigla == 'R'){
							//CASO LIVRO NAO POSSA SER EMPRESTADO
							printf("NÃO");
						} else {
							(p_livro->status+1)->sigla = 'R';
							strcpy(p_livro->status->RA, numRA);
			...
			gravaLivro(p_livro, "w+b", 1);
		}		

	} else {
		//CASO LIMITE DE EMPRESTIMOS ATINGIDO
	}
